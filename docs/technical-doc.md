...
该设计保持 OpenCode 风格的 Agent 中心模型：是否需要 Plan-and-Execute 不是单独 mode，而是 `AgentDef.workflow` 决定。权限仍由 AgentDef 的 `permissions` 和运行时 `--confirm/--no-confirm` 控制。
...
| 常量 | 值 | 说明 |
|------|----|------|
| `MAX_ITERATIONS` | 25 | build Agent 最大迭代轮数 |
| `MAX_MESSAGES` | 48 | 消息压缩阈值，避免单个开发任务过早触发 LLM 压缩 |
| `MAX_TOOL_RESULT_CHARS` | 10000 基线，按上下文窗口动态调整到 4000-48000 | 工具结果截断字符数 |
| `tool_result_line_limit()` | 300-2400 | 工具结果行数截断上限，按上下文窗口自适应 |
| `HARD_ITERATION_LIMIT` | 40 | 总工具调用硬上限（含多工具同一轮） |
| `MAX_RESPONSE_CHARS` | 6000 | Agent 最终响应截断字符数 |
...
4. `tools` 节点执行工具 → 权限规则集检查 → 工具去重检查 → 执行工具 → 追踪 `files_context` → 自适应截断+磁盘溢出 → `ToolMessage` 回传
   ...
   - **read_file 次数上限**：同一任务中超过 20 次读取会阻止继续读取，要求基于已收集信息总结
   - **glob/grep 去重**：相同 pattern 重复调用返回 `[Blocked]`；同类工具超过 6 次调用直接阻止
   - **run_shell 搜索拦截**：检测 grep/find/cat/ls/ack/ag/git grep 等搜索命令，阻止并提示使用专用工具
   - **run_shell 次数上限**：超过 8 次 run_shell 调用时阻止
...
`mode` 字段保留用于兼容旧配置；当前运行时行为以 `--agent` 和 `--confirm/--no-confirm` 为准。
...
> 注意：`mode` 字段已移除。`agent_name` 是行为唯一来源，权限/提示/工具均由 `AgentRegistry.get(agent_name)` 决定。
...
class AgentDef(BaseModel):
    ...
    display_name: str
    agent_mode: Literal["primary", "subagent"]  # 替代旧 mode 字段
...
| Agent | agent_mode | steps | confirm | tools | 权限 |
|-------|-----------|-------|---------|-------|------|
| build | primary | 25 | True | all | build 规则集 |
| plan | primary | 10 | False | all (权限运行时阻止写) | plan 规则集 |
| plan-execute | primary | 25 | True | all | build 规则集 + plan_execute 工作流 |
| explore | subagent | 10 | False | 只读工具 + skill/MCP 只读入口 | explore 规则集 |
| general | subagent | 20 | False | all | general 规则集 |
...
| `tool_result_char_limit()` | 根据 `context_window` 动态计算工具结果截断上限，范围 4000-48000 字符 |
| `tool_result_line_limit()` | 根据 `context_window` 动态计算工具结果行数上限，范围 300-2400 行 |
...
| 函数/常量 | 说明 |
|-----------|------|
| `MAX_RESPONSE_CHARS` | 响应截断上限 (6000 字符) |
| `FILE_SUMMARY_MAX_*` | 文件摘要常量 (600扫描行/20关键行/220字符) |
| `TASK_ITERATION_LIMITS` | 按任务类型的迭代上限字典 |
| `_extract_file_summaries()` | 兼容包装，实际委托给 `AgentContextManager` |
...
`build_system_prompt_with_context()` 接收任务类型解析后的有效预算，而不是直接使用 AgentDef 的默认 `steps`。例如 build Agent 默认 `steps=25`，但 `file_edit` 运行时提示会显示 `15` 轮预算，避免模型误以为仍有 25 轮可用。

当前状态评估遵循“报告是证据，不是唯一真相”的规则：当用户要求评估当前项目效果或基于历史报告继续优化时，Agent 需要把报告中的问题与当前源码、配置或测试交叉核对；如果当前实现已经覆盖报告建议，应标记为已解决/过期，而不是重复输出旧优化项。
...
#### 3.3.5 三层压缩 (`codepilot/agent/compaction.py`)

| 层级 | 函数 | 触发 | 方式 | 成本 |
|------|------|------|------|------|
| Pruning | `prune_tool_outputs()` | 80% 上下文 | 从后往前擦除旧工具输出，保留最近 40K tokens | 无 LLM |
| Compaction | `compact_messages()` | 80% + 消息过多 | LLM 总结旧消息，保留最近 2 轮 | 1 次 LLM |
...
#### 3.3.6 状态图构建 (`codepilot/agent/graph.py`)
...
#### 3.3.7 System Prompt (`codepilot/agent/prompts.py`)

由以下部分组成：
1. **基础指令**：定义 Agent 角色、能力、行为准则
2. **响应长度限制**：General Q&A 默认 ≤300 中文字符；Coding 默认 ≤600 词/1000 中文字符；Project Analysis 默认 ≤800 词；详细评估/方案类请求允许更完整展开
...
11. **项目分析规则**：简单分析 3-5 次调用；详细评估、优化方案或实现跟进最多使用任务预算
...
#### 3.3.8 项目指令文件 (`codepilot/context/instructions.py`)
...
class SessionInfo(BaseModel):     # 会话 (id, parent_id, title, agent, model, mode, ...)
```
...
#### 3.6.2 兼容层 (`codepilot/plugins/hooks.py`)

`hooks.py` 现在是向后兼容的 re-export shim，所有实现在 `manager.py` 中：
...
| `render_task_summary()` | `Table` | success/partial/error + 耗时/token/工具统计 |
...
| 层级 | 原方案 | 新方案 |
|------|--------|--------|
| 轻量 | 无 | Pruning — 无 LLM 调用，擦除旧工具输出 |
| 中等 | `_compact_messages` (无 LLM) | Compaction — 1 次 LLM 总结 |
| 重度 | 无 | Overflow — 紧急降级 |

新方案：Pruning 作为第一道防线节省 token；Compaction 用 LLM 总结保留关键信息；Overflow 处理极端情况。

### 5.3 权限规则集替代三模式开关

| 原方案 | 新方案 |
|--------|--------|
| 三模式 (plan/confirm/auto) | `PermissionRuleset` 规则集引擎 |
| 粗粒度: 写操作统一管控 | 细粒度: allow/ask/deny + glob 模式匹配 |
| 无路径级控制 | `pattern: "src/**"` 支持路径级权限 |
| 无命令级控制 | `pattern: "rm *"` 支持命令级权限 |
...
| 原方案 | 新方案 |
|--------|--------|
| 截断后内容丢失 | 截断内容保存到 `~/.codepilot/truncations/` |
| Agent 无法重新读取 | Agent 可通过 explore 子 Agent 读取完整文件 |
| 固定 head+tail | TruncationStore + 行数/字符数双限制 |
...
├── test_tools.py               # 文件工具、Shell、搜索、权限模式、Web 搜索
...
└── docs/
    ...
    ├── evaluation-report-20260607.md  # 线上评测报告
    ├── UPGRADE_PLAN.md          # 架构升级计划
    └── superpowers/
        └── specs/
            └── 2026-06-06-*.md  # 设计文档
```
