...
| 常量 | 值 | 说明 |
|------|----|------|
| `MAX_ITERATIONS` | 40 | build Agent 最大迭代轮数 |
...
| `HARD_ITERATION_LIMIT` | 80 | 总工具调用硬上限（含多工具同一轮） |
...
| `GRAPH_RECURSION_LIMIT` | 180 | LangGraph 超步上限 |
...
`build_system_prompt_with_context()` 接收任务类型解析后的有效预算，而不是直接使用 AgentDef 的默认 `steps`。例如 build Agent 默认 `steps=40`，但 `file_edit` 运行时提示会显示 `30` 轮预算，避免模型误判剩余执行空间。
