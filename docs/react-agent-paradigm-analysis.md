...
CodePilot 同时保留普通 ReAct 工作流和显式 Plan-and-Execute 工作流。普通开发任务可以直接进入 ReAct 循环；复杂任务可由 `plan-execute` Agent 先生成计划，再进入工具执行循环。
...
优势：复杂任务的目标、步骤和验证动作更清晰。代价：多一次规划调用，简单任务无需启用。
...
CodePilot 采用 ReAct、Plan-and-Execute、子 Agent 和权限规则集组合的混合方案，选择理由：

1. **CLI 场景需要可见执行过程**：ReAct 循环能让用户看到每步工具调用和结果
2. **复杂任务需要显式计划**：`plan-execute` 在执行前生成计划，降低方向漂移
3. **探索任务需要隔离上下文**：`explore` 子 Agent 可以独立读取和总结，不污染主对话
4. **真实项目需要权限边界**：规则集统一控制 allow/ask/deny，减少误执行风险

### 13.2 实测问题与对应范式

| 实测问题 | 当前处理 | 适合的范式 |
|----------|----------|-------------|
| 工具调用过多 | 迭代预算 + 工具结果压缩 + 任务状态提示 | ReAct / Plan-and-Execute |
| 重复读取同一文件 | files_context 追踪 + 已读上下文提示 | ReAct |
| 复杂任务方向漂移 | planner 节点先生成执行计划 | Plan-and-Execute |
| 大量探索污染主上下文 | `task` 工具派生 explore 子 Agent | 多 Agent |
| 修改后缺少验证 | 提示词和评测器要求最小验证 | ReAct / Plan-and-Execute |

### 13.3 LangSmith 驱动的范式选择

接入 LangSmith 后，可用数据驱动范式升级决策：

1. **建立基线**：收集不同 Agent 的迭代次数、token 消耗、成功率
2. **识别瓶颈**：哪些任务类型的迭代次数异常高？哪些任务失败率高？
3. **针对性升级**：
   - 高迭代任务 → Plan-and-Execute
   - 高失败率任务 → Reflexion
   - 多文件任务 → Supervisor
4. **A/B 对比**：同一任务类型对比不同 Agent 的 LangSmith trace
